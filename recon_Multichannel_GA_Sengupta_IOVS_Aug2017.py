...
# IF DEBUGGING, USE THE PYTHON DEBUGGER
import pdb
...
# Golden Angle Eye Recon Script. : Enter the Recon preferencs here for every file to be reconstructed.
# base_name : Name of the matlab file
# TR : Repetition Time
# nslices : No of slices/frames to be reconstructed
# npes_per_slice : No. of profiles per slice
# Step : Profiles step per image frame.

recon_list.append( { 'base_name' : 'raw_004', 'TR': 5.7e-3, 'nslices': 10, 'npes_per_slice' : int(128),'step' : int(10)  } )
'''
recon_list.append( { 'base_name' : 'raw_004', 'TR': 5.7e-3, 'nslices': 100, 'npes_per_slice' :  int(128),'step' : int(10) } )
recon_list.append( { 'base_name' : 'raw_005', 'TR': 5.7e-3, 'nslices': 100, 'npes_per_slice' :  int(128),'step' : int(10) } )
recon_list.append( { 'base_name' : 'raw_006', 'TR': 5.7e-3, 'nslices': 100, 'npes_per_slice' :  int(128),'step' : int(10) } )
recon_list.append( { 'base_name' : 'raw_007', 'TR': 5.7e-3, 'nslices': 100, 'npes_per_slice' :  int(128),'step' : int(10) } )
recon_list.append( { 'base_name' : 'raw_008', 'TR': 5.7e-3, 'nslices': 100, 'npes_per_slice' :  int(128),'step' : int(10) } )
recon_list.append( { 'base_name' : 'raw_010', 'TR': 5.7e-3, 'nslices': 100, 'npes_per_slice' :  int(128),'step' : int(10) } )
recon_list.append( { 'base_name' : 'raw_011', 'TR': 5.7e-3, 'nslices': 100, 'npes_per_slice' :  int(128),'step' : int(10) } )
recon_list.append( { 'base_name' : 'raw_012', 'TR': 5.7e-3, 'nslices': 100, 'npes_per_slice' :  int(128),'step' : int(10) } )
recon_list.append( { 'base_name' : 'raw_013', 'TR': 5.7e-3, 'nslices': 100, 'npes_per_slice' :  int(128),'step' : int(10) } )
'''


# INPUT DATA FILE : MAT File in the format of  nchan x nreadout_Points x nProfiles
...
def sparse_matrix_operator(K, grid_shape, osfactor=2, nhood_shape=None, verbose=False, combi=False, slice_prof_coef=None):
    ...
    if combi:
        if verbose:
            ...
            # WE ARE NOT USING THIS OPTION , USING kernel_kb instead
...
for recon in recon_list:
	  ...
	  data_file = data_path + base_name + '.mat'
	  
	  # EXAM CARD PARAMETERS
	  TR = recon['TR']
	  window_step = recon['step']    # Number of projections to step each recon window by    
	  radial_pct = 100
	  nslices = recon['nslices']  # number of slices in z direction
	  ...
	  golden_angle = True   # was data golden angle or linear?
	  fast_channel_combination = False  	# combine QBC channels before the recon?
	  ...
	  sum_of_squares_channel_combination = True                                   
	  ...
	  data_allchannels = mat['Data'].astype('complex64').T.copy('C')
	  npe, nro, nchan = data_allchannels.shape
	  print 'DATA INFO:\nnchan: %d\nnpe: %d\nnro: %d' % (nchan, npe, nro)
	  
	  
	  if fast_channel_combination:
		print 'combining QBC channels with fast channel combination'
		data_allchannels = do_fast_channel_combination(data_allchannels[:,:,0], data_allchannels[:,:,1], scale=fcc_scale, phase=fcc_phase)
		data_allchannels = reshape(data_allchannels, (-1, npe, nro))  # add singleton dimension for simplicity of following code
		nchan = 1
	  
	  elif sum_of_squares_channel_combination:
		nchan = data_allchannels.shape[2]
		data_allchannels = transpose(data_allchannels, [2,0,1])
	  ...
	  if crop_ro_oversampling:
		...
		data_allchannels = fftshift(ifft(fftshift(data_allchannels, axes=2), axis=2), axes=2)
		data_allchannels = data_allchannels[:,:,nro/4:-nro/4,]
		data_allchannels = fftshift(fft(fftshift(data_allchannels, axes=2), axis=2), axes=2).copy('C')
		nro = data_allchannels.shape[2] 
	  ...
	  if apply_phase_ramp:
		...
		for c in range(nchan):
			data_allchannels[c,:,:] = data_allchannels[c,:,:] * ramp
	  ...
	  # set up slice/profile divisions
	  slice_idx = 0
	  slice_center_z_mm_array = [0]
	  slice_center_projection_array = [128]
	  while True:
		slice_idx = slice_idx + 1
		next_projection_idx = slice_center_projection_array[0] + (slice_idx *window_step)
	  ...
	  im_gridding = zeros((nchan,nslices, nx, ny), 'complex64')
	  
	  for c in range(nchan):    
	  
		data = data_allchannels[c,:,:]
		...
		for s in range(nslices):   
			...
			#projections_slice_idx
			if(npes_per_slice%2)==0: # even nProjections
				pe_start = slice_center_projection_array[s] - (npes_per_slice/2 - 1);
				pe_end  = slice_center_projection_array[s] + npes_per_slice/2;
			else: # odd nProjections
				pe_start = slice_center_projection_array[s] - (npes_per_slice-1)/2
				pe_end  = slice_center_projection_array[s] + (npes_per_slice-1)/2
			...
			slice_data = reshape(slice_data, (-1, 1))    # [nro*npe_slice, nchan]
			X_gridding = A(slice_data, 2) 
			im_gridding[c,s,:,:] = squeeze(X_gridding)   #sqrt(sum(abs(X_gridding)**2, axis=2))
		
	  
	  print time.time()-t, 's elapsed'
	  
	  #mat['im_%d' % npes_per_slice] = im_gridding  # add reconstructed image to existing MAT file and save
	  im_file = data_path + base_name + '_' + str(npes_per_slice) + '_' + str(window_step) + '_PYTHONREC.mat'
	  #savemat(im_file, {'img':im_gridding[3700:3900,:,:]})
	  savemat(im_file, {'img':im_gridding[:,:,ncrop/2:-ncrop/2, ncrop/2:-ncrop/2]})
